...
function removeFirstOccurrences(str, value) {
  ...
  return firstIndex === -1
    ? str
    : str.substring(0, firstIndex) + str.substring(firstIndex + value.length);
}
...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, cur, index) => acc + str.charCodeAt(index), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return str.split('').filter((elem) => vowels.includes(elem.toLowerCase()))
    .length;
}
...
function findLongestWord(sentence) {
  const sortWordsArray = sentence
    .split(' ')
    .sort((a, b) => b.length - a.length);
  return sortWordsArray[0];
}
...
function encodeToRot13(str) {
  ...
  const abcRot13 =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm !?.,:;';
  const indexArr = str.split('').map((letter) => abc.split('').indexOf(letter));
  return indexArr.map((elem) => abcRot13[elem]).join('');
}
